<template>
  <div class="hairstyles">
    <div class="hairstyles_item" v-for="hairstyle in hairstyles">
      <router-link :to="{ name: 'Hairstyle', params: { id: hairstyle.id } }">
    ...
    <form v-if="form" id="file-form" enctype="multipart/form-data" action="http://127.0.0.1:8000/create_hairstyle" method="post" @submit.prevent="createNewHairstyle">
      <input type="text" name="email" v-model="email" style="display: none" />
      <table>
        <tbody>
        <tr>
          <th>Название прически:</th>
          <td><input type="text" name="title" placeholder="Введите название прически" required /></td>
        </tr>
        <tr>
          <th>Описание прически:</th>
          <td><input type="text" name="description" placeholder="Введите описание прически" required /></td>
        </tr>
        <tr>
          <th>Цена прически:</th>
          <td><input type="text" name="price" placeholder="Введите цену прически" required /></td>
        </tr>
        <tr>
          <th>Фотография:</th>
...
<script>
  ...
  export default {
    ...
    methods: {
      getHairstyles(){
        soso.search('hairstyles').then(resp => {
          this.hairstyles = resp.data.list;
        })
      },
      createNewHairstyle(e){
